...
| **2026-08-17** | **ISM Spatial Instance Matching & Dirty State on Removal** | Khi xóa phần tử khỏi `UInstancedStaticMeshComponent`, tuyệt đối không chỉ dựa vào `InstanceIndex` lưu ngầm (dễ bị lệch khi `Swap with Last`). Phải quét đối chiếu `Transform.GetLocation()` thực tế trong không gian 3D để xóa **chính xác 100% đúng ô**, đồng thời gọi `ISM->MarkRenderStateDirty()` để GPU làm mới mesh lập tức! |
| **2026-08-17** | **Protected Buildings & Safe Drag-to-Demolish** | Với các công trình đầu não (như `DistrictCenter`), đặt cờ `bCanBeDemolished = false` và chặn lệnh xóa. Cơ chế kéo chuột (Drag) chỉ áp dụng cho xóa dải đường `DirtPath`, với công trình bắt buộc phải Click đơn có chủ đích để tránh tai nạn quét chuột xóa nhầm! |
| **2026-08-17** | **Exact Door Adjacency Reachability** | Để nhận diện mạng lưới giao thông chuẩn xác trong game mô phỏng, thuật toán Reachability bắt buộc phải đối chiếu đường đi đến đúng tọa độ Ô Cửa (`GetDoorGridCoord()`) thay vì nhận bừa bãi 4 cạnh chu vi quanh móng! |
| **2026-08-17** | **Hologram Registration Prevention (bIsHologramPreview)** | Di chuyển con trỏ xem trước không được phép tự động gọi `RegisterBuilding` rồi `UnregisterBuilding` trong `BeginPlay`. Gắn cờ `bIsHologramPreview = true` để triệt tiêu hoàn toàn chi phí tính toán ngầm lặp lại mỗi frame! |
| **2026-08-19** | **2-Tier Root Cause Isolation & Blueprint Execution Flow Invariant** | Khi UI phản hồi bất thường (nhưng thế giới 3D đã phản hồi đúng), phân vùng lỗi tập trung vào Blueprint UI Graph. Tuyệt đối không để hở/đứt mạch chân `Exec Out` giữa các nhánh `Branch`, đảm bảo $100\%$ luồng chạy khép kín từ đầu đến cuối hàm để UI không bị kẹt trạng thái cũ! |
| **2026-08-19** | **Hierarchical Architecture Cache & Chrono Debug Trail** | Lưu trữ nhật ký kiến trúc phân cấp (`Architecture_Cache/`) theo từng Phase/Feat với 5 khối chuẩn (Mục tiêu, Thuật toán, Call-Graph, Biến trạng thái, Nhật ký sửa lỗi có ngày giờ) để phục vụ tra cứu Debug tức thì trong 30 giây! |
...
- **Kỹ năng đã thực hành xuất sắc**:
  ...
  - ✅ Lập trình Công cụ Cọ vẽ Runtime: Raycast 3D Cursor, Kéo thả dải đường `Drag-to-Build`, Phím tắt `R` xoay 90°.
  - ✅ Thuật toán Xóa An Toàn (Protected Demolish) & Khớp Tọa độ Thế giới 3D ISM (`Spatial Instance Matching`).
  - ✅ Hệ thống Mũi tên 3D Chỉ hướng cửa & Kết nối Mạng lưới Chuẩn xác Ô Cửa (`Exact Door Adjacency`).
- **Kỹ năng trọng tâm tiếp theo (Phase 4)**:
  - 🎯 Lập trình AI Hải Ly (Beaver AI FSM, Overlap Collision, A* Path Following).
  - 🎯 Vòng lặp Gameplay Khai thác & Cung ứng: Tìm Cây $\rightarrow$ Đốn Gỗ $\rightarrow$ Vác về Kho $\rightarrow$ Tiếp tế Móng xây dựng.
  - 🎯 Chu trình Nghỉ ngơi & Quản lý Dân số Hải ly (Beaver Lodges & Day/Night Cycle).
